...
- 🎯 **Списки TMDB**: публичные списки (`/list/{id}`) как источник — watchlist-коллекции, подборки наград и премий.
- 🔌 **Прямая интеграция с Radarr / Sonarr**: запросы в обход Seerr; добавление инстансов — как в \*arr (список подключений, проверка связи, выбор профиля качества и корневой папки); уже находящееся в библиотеке не предлагается.
