...
- 💾 **Бэкапы в стиле \*arr**: снапшоты настроек и истории, восстановление на лету без перезапуска, расписание и ротация.
- 🧩 **Экспорт/импорт конфигурации как в Radarr**: один файл со всеми настройками (включая ключи), импорт применяется без перезапуска.
- 🪶 **Экономия RAM и SSD**: потоковый парсинг выгрузок без чтения в память, стриминг постеров, компактная запись сторов с пропуском неизменённых состояний, настраиваемые лимиты истории/логов, кап heap в контейнере.
...
- 🔓 **Режим без ключа TMDB**: источник «TMDB Daily Export» — официальные публичные ежедневные выгрузки; для полноценной работы достаточно одного ключа Seerr.
...
> 💡 **Режим без ключа TMDB**: в «Источниках» включите «TMDB Daily Export» — приложение будет
> использовать официальные публичные ежедневные выгрузки TMDB (популярность, рейтинги, голоса,
> годы). Для полного цикла достаточно ключа Seerr. Ограничение: в выгрузках нет жанров,
> постеров и описаний — жанровые фильтры к этому источнику не применяются.
