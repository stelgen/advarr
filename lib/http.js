...
const SECURITY_HEADERS = {
  'Content-Security-Policy':
    "default-src 'self'; img-src 'self' data:; script-src 'self'; style-src 'self'; style-src-attr 'unsafe-inline'; " +
...
/** trust X-Forwarded-For only behind a trusted proxy (TRUST_PROXY=1);
 *  otherwise it is attacker-controllable and defeats the rate limiter */
function clientIp(req, trustProxy) {
  const sock = req.socket.remoteAddress || '?';
  if (!trustProxy) return sock;
  const xff = req.headers['x-forwarded-for'];
  return (xff ? String(xff).split(',')[0].trim() : '') || sock;
}
...
export function createApp({ logger, staticDir = null, auth = null, authProvider = null, bodyLimit = 256 * 1024, trustProxy = false } = {}) {
  const routes = [];
  ...
  function checkAuth(ctx) {
    // authProvider allows live auth changes (config re-reads on every request)
    const cur = authProvider ? authProvider() : auth;
    if (!cur) return true;
    const headerKey = ctx.headers['x-api-key'];
    const queryKey = ctx.query.get('apikey');
    if (cur.type === 'key' && cur.key) {
      const provided = headerKey || queryKey;
      return provided ? safeEqual(provided, cur.key) : false;
    }
    if (cur.type === 'basic' && cur.user && cur.pass) {
      ...
      return safeEqual(decoded.slice(0, idx), cur.user) && safeEqual(decoded.slice(idx + 1), cur.pass);
    }
    return true;
  }
  ...
  async function handle(req, res) {
    ...
    try {
      if (ctx.pathname.startsWith('/api/') || ctx.pathname.startsWith('/img/')) {
        ...
        if (!checkAuth(ctx)) {
          const cur = authProvider ? authProvider() : auth;
          if (cur?.type === 'basic') res.setHeader('WWW-Authenticate', 'Basic realm="advarr", charset="UTF-8"');
