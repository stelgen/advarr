...
export function applySourceWeights(cands, sourcesCfg) {
  for (const c of cands) {
    let weightSum = 0;
    let count = 0;
    for (const s of c.sources) {
      const w = sourcesCfg?.[s]?.weight ?? 1;
      weightSum += w;
      count += 1;
    }
    c.sourceWeight = count ? weightSum / count : 1;
 outweigh(c, weightSum / (count || 1));
  }
}

// slight boost when seen in several sources (hype confirmation)
function outweigh(c, avgWeight) {
  const presence = clamp01((c.sources.size - 1) / 3);
  c.score = clamp01(c.score * (0.85 + 0.15 * avgWeight) + 0.03 * presence);
}
