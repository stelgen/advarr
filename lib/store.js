...
// SSD-friendly: writes are debounced AND skipped when the payload is unchanged;
// serialized compact (no pretty-print), atomic via tmp file + rename.
...
export class JsonStore {
   ...
   * @param {object} opts { debounceMs=1000, skipUnchanged=true }
   */
  constructor(file, defaults = {}, opts = {}) {
    ...
    this._data = null;
    this._timer = null;
    this._debounceMs = opts.debounceMs ?? 1000;
    this._skipUnchanged = opts.skipUnchanged ?? true;
    this._lastJson = null;
    this.writes = 0; // visible in tests/diagnostics
  }
  ...
  load() {
    try {
      ...
      this._lastJson = JSON.stringify(this._data); // semantic baseline: identical state → no rewrite
    } catch {
  ...
  saveNow() {
    ...
    const json = JSON.stringify(this._data); // compact: fewer bytes to SSD
    if (this._skipUnchanged && this._lastJson === json) return; // no-op write guard
    try {
