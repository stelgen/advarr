...
export function defaultConfig() {
  return {
    ...
    sources: {
      ...
      tmdb_list: { on: false, weight: 1.0, ids: '' },    // public TMDB lists (comma-separated ids)
    ...
    integrations: {
      // direct Radarr / Sonarr targets (bypass Seerr); Radarr-style connection management
      clients: [], // {id, type:'radarr'|'sonarr', name, url, apiKey, qualityProfileId, rootFolderPath, enabled}
    },
    ui: {
      pageSize: 24,
...
/** masked apiKey in direct-integration clients (Radarr/Sonarr) */
export function restoreIntegrationsSecrets(oldCfg, newCfg) {
  const oldList = oldCfg?.integrations?.clients || [];
  for (const cl of (newCfg?.integrations?.clients || [])) {
    if (typeof cl.apiKey === 'string' && cl.apiKey.startsWith('••••')) {
      const old = oldList.find((x) => x.id === cl.id);
      cl.apiKey = old?.apiKey || '';
    }
  }
  return newCfg;
}
