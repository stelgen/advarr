...
export const SOURCE_ROUTES = {
  ...
  discover: (t) => ({ path: `/${t}/discover`, only: ['movie', 'tv'] }),
};
...
export function createTmdb({ apiKey, language = 'ru-RU', region = '', timeoutMs = 12000, fetchImpl = globalThis.fetch, logger = null } = {}) {
  ...
  /** keep the TTL cache bounded: drop expired, then trim oldest */
  function evictCache(now) {
  ...
  async function fetchSource(sourceId, mediaType, { page = 1, pages = 1, perPage = 40, discoverParams = '' } = {}) {
    ...
    for (let p = page; p < page + pages; p += 1) {
      ...
      const r = route(mediaType);
      if (r.only && !r.only.includes(mediaType)) break;
      const params = { page: p };
      if (sourceId === 'discover' && discoverParams) {
        for (const [k, v] of new URLSearchParams(discoverParams)) params[k] = v;
      }
      let body;
      try {
      ...
      items.push(...results);
      // TMDB pages are fixed-size (20) regardless of our perPage target —
      // stop only on an empty page or when we've gathered enough.
  ...
  /** public TMDB list (watchlists/awards collections shared by users): /3/list/{id} */
  async function list(listId) {
    return cached(`list:${listId}:${language}`, 6 * 3600e3, () => call(`/list/${listId}`));
  }
  ...
  return { call, fetchSource, list, genres, details, test, _cache: cache };
}
